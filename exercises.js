...
function sumArray(numbers, cb) {
	...
	cb(numbers.reduce(function(sum, num) { return sum + num; }));
}
...
function map(arr, cb) {
	...
	return arr.map(function(num) { return cb(num); });
}
...
function nFactorial(n) {
	...
	//example: the factorial of 3 is 6 (3 * 2 * 1)
...
function cacheFunction(cb) {
	...
	var cache = {};
	return function(x) {
		return cache.hasOwnProperty(x) ? cache[x] : cache[x] = cb(x);
	}; 
}
